...
*/
	public void renderSprite(Map map,Renderer renderer, int spriteNumber){
		int sn=spriteNumber;

		while(sn<0){ // if negative
			sn+=sprites.size();
		}
		
		if(sn>=sprites.size()){
				sn=spriteNumber%(sprites.size());
		}
		
		Sprite sprite=sprites.get(sn);
		sprite.render(map,renderer);
	}
	...
	public void correlate(MapObject obj) {
